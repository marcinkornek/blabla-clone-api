...
RSpec.describe Ride, type: :model do
  ...
  describe "scopes" do
    ...
    describe ".to_city(latitude, longitude)" do
      let(:location) { FactoryGirl.create(:location) }
      let!(:ride) { FactoryGirl.create(:ride, destination_location_id: location.id) }
    ...
    describe ".on_day(date)" do
      let!(:ride) { FactoryGirl.create(:ride, start_date: Time.zone.parse("2016-05-19 10:30:14")) }
    ...
    describe ".without_full" do
      let!(:ride) { FactoryGirl.create(:ride, places: 2) }
    ...
    describe ".future" do
      let!(:ride) { FactoryGirl.create(:ride, start_date: start_date) }
    ...
    describe ".other_users_rides(user)" do
      let!(:ride) { FactoryGirl.create(:ride, driver: driver) }
